import lldb as lldb_module  # Alias to avoid conflicts
import math
import logging
...
def slice_summary(value, internal_dict):
    ...
    capacity = value.GetChildMemberWithName("cap").unsigned
    data   = value.GetChildMemberWithName("data")
    pointee   = data.deref
    type_name = pointee.type.GetDisplayTypeName()

    return f"[len:{length}/cap:{capacity}]{type_name}"
...
class MapChildProvider:
    ...
    def get_child_at_index(self, index):
        ...
        if index == self.num_children()-1:
            cap_data = lldb_module.SBData.CreateDataFromInt(cap)
...
def __lldb_init_module(debugger, unused):
    debugger.HandleCommand(
        "type summary add --recognizer-function --python-function lldb.slice_summary lldb.is_slice_type"
    )
    debugger.HandleCommand(
        "type synth add --recognizer-function --python-class lldb.SliceChildProvider lldb.is_slice_type"
    )
